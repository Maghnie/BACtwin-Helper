...
function Funktion() {
  ...
  const fetchDaten = async () => {
    const res = await axios.get(`${API_URL}/funktion`);
    setDaten(res.data);
  };

  const addDatensatz = async () => {
    if (!form.kuerzel || !form.bezeichnung) return;
    const res = await axios.post(`${API_URL}/funktion`, form);
  ...
  const saveDatensatz = async (id) => {
    ...
    await axios.put(`${API_URL}/funktion/${id}`, d);
  };
  ...
  return (
    <div>
      ...
      */}
      ...
      {filtered.length === 0 && <p style={{ color: "#666" }}>Keine Funktionen gefunden.</p>}
    </div>
  );
}
