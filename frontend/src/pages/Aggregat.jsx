...
function Aggregat() {
  ...
  const fetchDaten = async () => {
    const res = await axios.get(`${API_URL}/aggregat`);
    setDaten(res.data);
  };

  const addDatensatz = async () => {
    if (!form.kuerzel || !form.bezeichnung) return;
    const res = await axios.post(`${API_URL}/aggregat`, form);
  ...
  const saveDatensatz = async (id) => {
    ...
    await axios.put(`${API_URL}/aggregat/${id}`, d);
  };
...
*/}
      ...
      {filtered.length === 0 && (
        <p style={{ marginTop: 10, color: "#666" }}>Keine Aggregate gefunden.</p>
      )}
    </div>
  );
}

export default Aggregat;
