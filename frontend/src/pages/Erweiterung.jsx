...
function Erweiterung() {
  ...
  const fetchDaten = async () => {
    const res = await axios.get(`${API_URL}/erweiterung`);
    setDaten(res.data);
  };

  const addDatensatz = async () => {
    if (!form.kuerzel || !form.bezeichnung) return;
    const res = await axios.post(`${API_URL}/erweiterung`, form);
  ...
  const saveDatensatz = async (id) => {
    ...
    await axios.put(`${API_URL}/erweiterung/${id}`, d);
  };
  ...
  return (
    <div>
      ...
      */}
              ...
              <tr key={d.id}>
                <td>
                  <input
                    ...
                    style={{ width: "100%", boxSizing: "border-box", border: "1px solid #ddd", borderRadius: 4, padding: "4px" }}
                  />
                </td>
                <td>
                  <input
      ...
      {filtered.length === 0 && <p style={{ color: "#666" }}>Keine Erweiterung gefunden.</p>}
    </div>
  );
}
