...
function Mediumposition() {
  ...
  const fetchDaten = async () => {
    const res = await axios.get(`${API_URL}/mediumposition`);
    setDaten(res.data);
  };

  const addDatensatz = async () => {
    if (!form.kuerzel || !form.bezeichnung) return;
    const res = await axios.post(`${API_URL}/mediumposition`, form);
  ...
  const saveDatensatz = async (id) => {
    ...
    await axios.put(`${API_URL}/mediumposition/${id}`, d);
  };
  ...
  return (
    <div>
      ...
      */}
        ...
        <table style={{ width: "100%", borderCollapse: "collapse", minWidth: 500 }}>
          <thead>
            <tr style={{ background: "#f3f4f6" }}>
              ...
              <th style={{ width: "80px" }}>Bezeichnung</th>
              <th>Beschreibung</th>
      ...
      {filtered.length === 0 && (
        <p style={{ marginTop: 10, color: "#666" }}>Keine Mediumpositionen gefunden.</p>
      )}
    </div>
  );
}
